const puppeteer = require("puppeteer");
const fs = require("fs");
const path = require("path");

async function scrapeShopifyReviews(url, browser) {
  const page = await browser.newPage();

  try {
    await page.goto(url, { waitUntil: "networkidle2" });

    // Wait for potential dynamic content and scroll to load reviews
    await new Promise((resolve) => setTimeout(resolve, 3000));

    // Scroll down to trigger any lazy loading
    await page.evaluate(() => {
      window.scrollTo(0, document.body.scrollHeight / 2);
    });

    await new Promise((resolve) => setTimeout(resolve, 2000));

    // Scroll down more to ensure reviews are loaded
    await page.evaluate(() => {
      window.scrollTo(0, document.body.scrollHeight);
    });

    await new Promise((resolve) => setTimeout(resolve, 2000));

    // First, expand all "show more" buttons to get full review content
    let showMoreButtons = await page.$$(
      '[data-testid="review-item-show-more-button"]'
    );
    console.log(`Found ${showMoreButtons.length} "show more" buttons`);

    for (const button of showMoreButtons) {
      try {
        await button.click();
        await new Promise((resolve) => setTimeout(resolve, 800)); // Wait for content to expand
      } catch (error) {
        console.log("Could not click show more button:", error.message);
      }
    }

    // Try multiple alternative selectors for expand buttons
    const alternativeSelectors = [
      'button[aria-expanded="false"]',
      "button[data-expand]",
      "button[data-show-more]",
      ".show-more",
      ".expand-button",
    ];

    for (const selector of alternativeSelectors) {
      try {
        const buttons = await page.$$(selector);
        console.log(
          `Found ${buttons.length} buttons with selector: ${selector}`
        );

        for (const button of buttons) {
          try {
            await button.click();
            await new Promise((resolve) => setTimeout(resolve, 500));
          } catch (error) {
            // Silently continue if button can't be clicked
          }
        }
      } catch (error) {
        // Invalid selector, continue to next one
      }
    }

    // Additional approach: look for any clickable elements within review containers that might expand content
    await page.evaluate(() => {
      const reviewElements = document.querySelectorAll(
        "[data-merchant-review]"
      );
      reviewElements.forEach((review) => {
        // Look for any clickable elements that might expand content
        const clickables = review.querySelectorAll(
          'button, a, [role="button"], .clickable'
        );
        clickables.forEach((clickable) => {
          const text = clickable.textContent?.toLowerCase() || "";
          if (
            text.includes("more") ||
            text.includes("expand") ||
            text.includes("show") ||
            text.includes("read")
          ) {
            try {
              clickable.click();
            } catch (e) {
              // Ignore click errors
            }
          }
        });
      });
    });

    // Wait for all expansions to complete
    await new Promise((resolve) => setTimeout(resolve, 2000));

    const reviews = await page.evaluate(() => {
      // Use the correct selector based on the actual HTML structure
      const reviewElements = document.querySelectorAll(
        "[data-merchant-review]"
      );
      console.log(`Found ${reviewElements.length} review elements`);

      return Array.from(reviewElements).map((review, index) => {
        console.log(`Processing review ${index + 1}`);

        // Extract star rating by looking at the aria-label or counting stars in the rating container
        const ratingContainer = review.querySelector(
          '[aria-label*="out of 5 stars"]'
        );
        let rating = "";

        if (ratingContainer) {
          // Extract rating from aria-label like "4 out of 5 stars"
          const ariaLabel = ratingContainer.getAttribute("aria-label");
          const match = ariaLabel.match(/(\d+)\s+out of \d+ stars/);
          if (match) {
            rating = match[1];
          }
        } else {
          // Fallback: count filled stars within the rating container only
          const ratingDiv = review.querySelector(
            ".tw-flex.tw-relative.tw-space-x-0\\.5"
          );
          if (ratingDiv) {
            const filledStars = ratingDiv.querySelectorAll(
              ".tw-fill-fg-primary"
            );
            rating = filledStars.length.toString();
          }
        }

        // Get review content - try multiple selectors for expanded content
        let content = "";

        // First try the expanded content selector
        const expandedContent = review.querySelector(
          'p.tw-break-words[data-expanded="true"], p.tw-break-words:not([data-truncated])'
        );
        if (expandedContent) {
          content = expandedContent.textContent?.trim() || "";
        } else {
          // Fallback to regular content selector
          const regularContent = review.querySelector("p.tw-break-words");
          if (regularContent) {
            content = regularContent.textContent?.trim() || "";
          }
        }

        // Try alternative content selectors if still empty or truncated
        if (!content || content.endsWith("...") || content.endsWith("…")) {
          const alternativeSelectors = [
            ".review-content",
            "[data-review-content]",
            ".tw-break-words span",
            "p:not(.tw-text-heading-xs):not(.tw-text-body-xs)",
            ".review-text",
          ];

          for (const selector of alternativeSelectors) {
            const element = review.querySelector(selector);
            if (
              element &&
              element.textContent?.trim() &&
              !element.textContent.trim().endsWith("...") &&
              !element.textContent.trim().endsWith("…")
            ) {
              content = element.textContent.trim();
              break;
            }
          }
        }

        // Get store name from the heading element
        const storeName =
          review
            .querySelector(".tw-text-heading-xs.tw-text-fg-primary")
            ?.textContent?.trim() || "";

        // Get date from the tertiary text element
        const dateElement = review.querySelector(
          ".tw-text-body-xs.tw-text-fg-tertiary"
        );
        const date = dateElement?.textContent?.trim() || "";

        // Get the metadata container
        const metadataContainer = review.querySelector(
          ".tw-order-2.lg\\:tw-order-1"
        );
        let country = "";
        let appUsageDuration = "";

        if (metadataContainer) {
          // Get direct child divs that contain the metadata
          const childDivs = Array.from(metadataContainer.children).filter(
            (child) =>
              child.tagName === "DIV" && child.textContent.trim() !== storeName
          );

          // Extract text from each div, cleaning up whitespace
          const cleanTexts = childDivs
            .map((div) => div.textContent?.replace(/\s+/g, " ").trim())
            .filter((text) => text && text !== storeName);

          if (cleanTexts.length >= 1) {
            country = cleanTexts[0] || "";
          }
          if (cleanTexts.length >= 2) {
            appUsageDuration = cleanTexts[1] || "";
          }
        }

        return {
          rating: rating.toString(),
          storeName,
          country,
          appUsageDuration,
          date,
          content,
          helpful: "", // Not visible in the structure
        };
      });
    });

    await page.close();
    return reviews;
  } catch (error) {
    await page.close();
    throw error;
  }
}

function generateCSV(reviews) {
  const headers = [
    "Rating",
    "Store Name",
    "Country",
    "App Usage Duration",
    "Date",
    "Content",
  ];
  const csvContent = [
    headers.join(","),
    ...reviews.map((review) =>
      [
        review.rating,
        `"${review.storeName.replace(/"/g, '""')}"`,
        `"${review.country.replace(/"/g, '""')}"`,
        `"${review.appUsageDuration.replace(/"/g, '""')}"`,
        `"${review.date.replace(/"/g, '""')}"`,
        `"${review.content.replace(/"/g, '""')}"`,
      ].join(",")
    ),
  ].join("\n");

  return csvContent;
}

function generateFileName(format = "csv") {
  const today = new Date();
  const dateString = today.toISOString().split("T")[0];
  const timestamp = today.getTime();
  return `shopify-reviews-${dateString}-${timestamp}.${format}`;
}

function generateJSON(reviews) {
  return JSON.stringify(reviews, null, 2);
}

async function main() {
  const url = process.argv[2];
  const format = process.argv[3] || "csv";
  const pages = parseInt(process.argv[4]) || 1;

  if (!url) {
    console.error(
      "Usage: node shopify-review-scraper.js <shopify-app-review-url> [csv|json] [pages]"
    );
    console.error("Examples:");
    console.error(
      '  node shopify-review-scraper.js "https://apps.shopify.com/flow/reviews?ratings[]=4"'
    );
    console.error(
      '  node shopify-review-scraper.js "https://apps.shopify.com/flow/reviews?ratings[]=4" json'
    );
    console.error(
      '  node shopify-review-scraper.js "https://apps.shopify.com/flow/reviews?ratings[]=4" json 3'
    );
    process.exit(1);
  }

  if (!["csv", "json"].includes(format.toLowerCase())) {
    console.error('Error: Output format must be either "csv" or "json"');
    process.exit(1);
  }

  if (pages < 1 || pages > 1000) {
    console.error("Error: Number of pages must be between 1 and 1000");
    process.exit(1);
  }

  try {
    let allReviews = [];
    const startTime = Date.now();

    // Create browser instance once and reuse it
    const browser = await puppeteer.launch({ headless: true });

    try {
      for (let page = 1; page <= pages; page++) {
        let pageUrl = url;

        // Add page parameter to URL
        if (pages > 1) {
          const separator = url.includes("?") ? "&" : "?";
          pageUrl = `${url}${separator}page=${page}`;
        }

        console.log(`Scraping reviews from page ${page}:`, pageUrl);
        const pageStartTime = Date.now();
        const reviews = await scrapeShopifyReviews(pageUrl, browser);
        const pageEndTime = Date.now();
        const pageTime = ((pageEndTime - pageStartTime) / 1000).toFixed(2);

        if (reviews.length === 0) {
          console.log(`No reviews found on page ${page}. Stopping pagination.`);
          break;
        }

        allReviews = allReviews.concat(reviews);
        console.log(
          `Page ${page}: Found ${reviews.length} reviews in ${pageTime}s (Total: ${allReviews.length})`
        );

        // Add delay between pages to be respectful
        if (page < pages) {
          console.log("Waiting 2 seconds before next page...");
          await new Promise((resolve) => setTimeout(resolve, 2000));
        }
      }
    } finally {
      // Always close browser, even if there's an error
      await browser.close();
    }

    if (allReviews.length === 0) {
      console.log(
        "No reviews found. The page structure might have changed or no reviews exist."
      );
      return;
    }

    let content;
    let fileName;

    if (format.toLowerCase() === "json") {
      content = generateJSON(allReviews);
      fileName = generateFileName("json");
    } else {
      content = generateCSV(allReviews);
      fileName = generateFileName("csv");
    }

    const filePath = path.join(__dirname, fileName);
    fs.writeFileSync(filePath, content, "utf8");

    const endTime = Date.now();
    const totalTime = ((endTime - startTime) / 1000).toFixed(2);

    console.log(
      `Successfully scraped ${allReviews.length} reviews from ${Math.min(
        pages,
        allReviews.length > 0 ? Math.ceil(allReviews.length / 10) : 1
      )} pages`
    );
    console.log(`Total time: ${totalTime}s`);
    console.log(
      `Output saved to: ${fileName} (${format.toUpperCase()} format)`
    );
  } catch (error) {
    console.error("Error scraping reviews:", error.message);
    process.exit(1);
  }
}

main();
